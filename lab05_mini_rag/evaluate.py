...
def main():
    ...
    for r in read(ROOT/'results/predictions.jsonl'):
        ...
        rows.append({'question_id':r['question_id'],'split':g['split'],'mode':r['mode'],'valid':r['valid'],'answer_correct':correct,'answer_exact':expected==actual and r['valid'],'answerable_correct':r['valid'] and p['answerable']==g['answerable'],'gold_answerable':g['answerable'],'pred_answerable':p['answerable'],'evidence_precision':precision,'evidence_recall':recall,'evidence_f1':2*precision*recall/(precision+recall) if precision+recall else 0,'all_gold_in_context':b.issubset(r['context_ids']),'context_answerable':g['answerable'] and b.issubset(r['context_ids']),'context_answerable_correct':r['valid'] and p['answerable']==(g['answerable'] and b.issubset(r['context_ids'])),'seconds':r.get('elapsed_seconds',0),'input_tokens':usage.get('prompt_tokens',0),'output_tokens':usage.get('completion_tokens',0)})
    df=pd.DataFrame(rows);df.to_csv(ROOT/'results/scores.csv',index=False)
    metrics=df.groupby(['split','mode']).agg(n=('valid','size'),json_validity=('valid','mean'),answer_accuracy=('answer_correct','mean'),answer_exact=('answer_exact','mean'),answerable_accuracy=('answerable_correct','mean'),context_answerable_accuracy=('context_answerable_correct','mean'),evidence_precision=('evidence_precision','mean'),evidence_recall=('evidence_recall','mean'),evidence_f1=('evidence_f1','mean'),seconds_mean=('seconds','mean'),input_tokens_mean=('input_tokens','mean'),output_tokens_mean=('output_tokens','mean'))
    ...
    candidates={
        ...
        'answerability':df[~df['context_answerable_correct']],
    }
    ...
    for cat,frame in candidates.items():
        ...
        for _,r in (frame.tail(5) if cat=='answerability' else frame.head(5)).iterrows():
