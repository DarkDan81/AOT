...
print(inspect.getsource(tokens) + '\\n\\n' + inspect.getsource(top5))
''')
md('''### Пояснение реализации поиска

Функция `tokens(text)` подготавливает текст к поиску. `tokenize` из библиотеки razdel разбивает его на токены, а `lower()` приводит их к нижнему регистру, чтобы написание с заглавной буквы не мешало совпадению. Условие `any(c.isalnum() for c in x.text)` сохраняет токены хотя бы с одной буквой или цифрой и исключает отдельные знаки пунктуации. Лемматизация и удаление стоп-слов не выполняются: разные формы одного слова могут остаться разными токенами.

Функция `top5(document, question)` получает документ и вопрос. Принадлежность вопроса документу заранее задана через `document_id`, поэтому поиск выполняется среди 24 абзацев этого документа. Эталонные ответы и доказательные ID при ранжировании не используются.

1. `ps = document['paragraphs']` извлекает список абзацев вместе с их стабильными идентификаторами.
2. `[tokens(p['text']) for p in ps]` преобразует каждый абзац в список токенов. `BM25Okapi(...)` рассчитывает статистику слов и длин абзацев. В этой реализации объект поиска создаётся при каждом вызове функции; отдельный индекс на диске не сохраняется.
3. `get_scores(tokens(question))` рассчитывает оценку каждого абзаца по словам вопроса. BM25 учитывает совпадения слов, их редкость среди абзацев, частоту внутри абзаца и его длину. Влияние повторений насыщается, а длина нормализуется, чтобы большой абзац не получал преимущество только за счёт объёма. Это лексический поиск без обучения нейросети.
4. `sorted(range(len(ps)), key=lambda i: (-scores[i], i))` сортирует индексы абзацев. Python сортирует по возрастанию, поэтому минус перед оценкой ставит лучшие совпадения первыми. Второй элемент `i` сохраняет исходный порядок при равных оценках и делает выбор однозначным.
5. `[:5]` оставляет ровно пять лучших абзацев, как требуется в задании. Выражение `ps[i]` возвращает сами абзацы с текстом и ID, чтобы их можно было передать модели и затем проверить ссылки в ответе.

Для Q005 правильный ответ требует шести отдельных абзацев: 16–21. Поиск возвращает `[21, 17, 16, 18, 19]`; абзац 20 с разрешением E501 не попадает в контекст. Даже идеальное ранжирование не вместит шесть обязательных абзацев в top-5. Этот пример показывает ограничение размера выдачи: далее генератор должен определить, что сведений недостаточно для полного ответа, и отказаться согласно промпту.
''')
code('''
...
print(inspect.getsource(Answer) + '\\n\\n' + inspect.getsource(validate))
''')
md('''### Пояснение проверки ответа

Класс `Answer` задаёт три обязательных поля: `answer` — строка или null, `evidence_ids` — список целых чисел, `answerable` — логическое значение. `strict=True` запрещает автоматическое приведение типов, а `extra='forbid'` — дополнительные поля.

Метод `coherent()` проверяет согласованность полей: положительный ответ требует непустого текста и ссылок; отказ требует null и пустого списка. Повторяющиеся идентификаторы отклоняются.

Функция `validate(content, ids)` разбирает JSON через `parse_json`, проверяет его моделью `Answer` и убеждается через `issubset(ids)`, что все ссылки входят в фактически переданный контекст. При нарушении возникает исключение; при успехе `model_dump()` возвращает проверенный словарь. Ответ автоматически не исправляется.

Эта проверка подтверждает структуру и допустимость ссылок, но не истинность и полноту ответа. Следующие три примера проверяют валидатор и не являются ответами LLM.
''')
code('''
examples = [
