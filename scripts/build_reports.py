...
def cover(doc,num,title):
    ...
    individual=num in {1,3,4}
    p=clone(12,('Обучающийся: ' if individual else 'Обучающиеся: ')+'Дущенко Даниил Александрович, К3341');p.paragraph_format.space_before=Pt(62 if individual else 52)
    if not individual:clone(12,'Коваленко Евгений Юрьевич, К3341')
...
def build(folder):
    ...
    doc.core_properties.author='Дущенко Даниил Александрович'+('' if num in {1,3,4} else '; Коваленко Евгений Юрьевич');doc.core_properties.title=f'Лабораторная работа № {num}. {title}';doc.core_properties.subject='Автоматическая обработка текста';doc.core_properties.comments='Группа К3341. Методика и результаты экспериментов описаны в отчете.'
